% Main script for processing raw data and performing spectroscopy
% All three subfolders must be added to path before running this script
%   - .\+bauerParams
%   - .\export_fig
%   - .\supplementaryFunctions

% Inputs:
%   excelfile:  Database file with list of mice/runs and the associated 
%               experiment parameters (see Example.xlsx)
%   rows:       row number(s) within the excel file corresponding to the 
%               mice to be analyzed

% Outputs:
%   xform_datahb:   contains information on the changes in concentrations 
...
% Specify inputs here
excelFile = "\path_to_excel_file"; % specify location of .xlsx file
excelRows = [1,2,3]; % excel rows to process
...
%Note that unique only takes characters! This makes it so that we only 
% do landmark for one of the runs.
...
for runInd=mouse_ind %for each mouse
    ...
    if  ~contains(runInfo.system,'EastOIS2')
        ...
        if ~exist(runInfo.saveMaskFile,'file') || (~ismember('xform_StimROIMask', ...
                variableInfo) && runInfo.session =="stim") 
            %%For laser case, we have a landmarksandMask without stimMask 
            % before the processing script
            ...
            if runInfo.session =="stim"
                ...
                for jj=1:runInfo.numCh
                    tmp(:,:,jj)=(mean(raw(:,:,jj,runInfo.stimStartTime* ...
                        runInfo.samplingRate+runInfo.darkFramesInd(end)+1:end),4)-...
                        mean(raw(:,:,jj,runInfo.darkFramesInd(end)+ ...
                        1:runInfo.darkFramesInd(end)+runInfo.stimStartTime* ...
                        runInfo.samplingRate),4)).^2; %mean from dark frames to end
            ...
            [isbrain,xform_isbrain,I,WL,xform_WL] = getMask(raw,runInfo.darkFramesInd, ...
                runInfo.invalidFramesInd,runInfo.rgbInd);
            %Make a save folder
            ...
            elseif ~exist(runInfo.saveMaskFile,'file') 
                ...
                save(runInfo.saveMaskFile,'xform_WL','isbrain','I','xform_isbrain', ...
                    'WL','xform_StimROIMask','StimROIMask','-v7.3');
            else 
        ...
        if ~exist(runInfo.saveMaskFile,'file') || (~ismember('xform_StimROIMask', variableInfo) ...
                && runInfo.session =="stim")
            sessionInfo = sysInfo(runInfo.system);

            tmp=matfile(runInfo.rawFile{1});
            try
                raw_unregistered=tmp.raw_unregistered(:,:,1:200*runInfo.numCh);
            catch
                raw_unregistered=tmp.raw_unregistered(:,:,:,1:200);
            end
            raw_unregistered=reshape(raw_unregistered,size(raw_unregistered,1), ...
                size(raw_unregistered,2),runInfo.numCh,[]);
            ...
            if runInfo.session =="stim"
                ...
                fh = figure('units','normalized','outerposition',[0 0 1 1]);
                for jj=1:runInfo.numCh
                    tmp(:,:,jj)=(mean(raw_unregistered(:,:,jj,runInfo.stimStartTime* ...
                        runInfo.samplingRate+runInfo.darkFramesInd(end)+1:end),4)-...
                        mean(raw_unregistered(:,:,jj,runInfo.darkFramesInd(end)+ ...
                        1:runInfo.darkFramesInd(end)+runInfo.stimStartTime* ...
                        runInfo.samplingRate),4)).^2; %mean from dark frames to end
                    subplot(1,runInfo.numCh,jj)
                    imagesc(squeeze(tmp(:,:,jj)))
            ...
            firstFrame_cam1  = squeeze(raw_unregistered(:,:,sessionInfo.rgb(2), ...
                runInfo.darkFramesInd(end)+1)); % First non dropped frame

            clear raw_unregistered tmp
            tmp=matfile(runInfo.rawFile{2}); 
            try
                raw_unregistered=tmp.raw_unregistered(:,:,1:200*runInfo.numCh);
            catch
                raw_unregistered=tmp.raw_unregistered(:,:,:,1:200);
            ...
            firstFrame_cam2  = squeeze(raw_unregistered(:,:,sessionInfo.rgb(1), ...
                runInfo.darkFramesInd(end)+2));

            clear raw_unregistered tmp

            if ~strcmp(previousDate,currentDate) && ~exist(fullfile(runInfo.saveFolder, ...
                    strcat(runInfo.recDate,'-tform.mat')),'file')
                ind_recDate= strfind(runInfo.rawFile{1},runInfo.recDate);
                rawDataLoc = runInfo.rawFile{1}(1:ind_recDate(2)-1);
                if exist([rawDataLoc,runInfo.recDate,'-gridWL-cam1.mat'],'file')
                    ...
                    clear raw_unregistered
                    [mytform,fixed_cam1,registered_cam2] = getTransformation( ...
                        firstFrame_cam1,firstFrame_cam2);
                end
                if ~exist(runInfo.saveFolder, 'dir')
                    mkdir(runInfo.saveFolder)
                end
            ...
            disp([runInfo.saveHbFile,' Label'])
            [isbrain,xform_isbrain,I,seedcenter,WLcrop,xform_WLcrop,xform_WL] ...
            = getLandMarksandMask_Zyla(WL);
            %Make a save folder
            if ~exist(runInfo.saveFolder, 'dir')
                mkdir(runInfo.saveFolder);
            end
...
for runInd = 1:runNum
    ...
    %Check if these already exist
        ...
        fcExist=[runInfo.saveFilePrefix '-seedFC-' num2str(fRange_ISA(1)) '-' 
            num2str(fRange_ISA(2)) ];
        fcExist(strfind(fcExist,'.')) = 'p';
        fcExist=[fcExist '.mat'];
    %If is this already processed
    if exist( runInfo.saveHbFile,'file')  && ~( (exist(stimExist,'file') && ...
            exist(stimExist2,'file') && strcmp(runInfo.session,'stim') ) ||  ...
            (exist(fcExist,'file') && strcmp(runInfo.session,'fc')) ) 
    ...
    elseif ((exist(stimExist,'file') && exist(stimExist2,'file') && strcmp(runInfo.session,'stim') ) ...
            ||  (exist(fcExist,'file') && strcmp(runInfo.session,'fc') ))
        ...
        if ~contains(runInfo.system,'EastOIS2')
            ...
            raw=FindandFixDroppedFrames(raw,runInfo);
        else
            ...
            load(fullfile(runInfo.saveFolder,strcat(runInfo.recDate,'-tform.mat')),'mytform')
            raw  = registerCam2andCombineTwoCams(binnedRaw_cam1,binnedRaw_cam2, ...
                mytform,sessionInfo.cam1Chan,sessionInfo.cam2Chan);
            save(fullfile(runInfo.saveFolder,strcat(runInfo.recDate,'-', ...
                runInfo.mouseName,'-',runInfo.session,num2str(runInfo.run))),'raw','-v7.3')
        end

        %DARK FRAME REMOVAL
        if ~isempty(runInfo.darkFramesInd)
            runInfo.darkFramesInd(runInfo.darkFramesInd == runInfo.invalidFramesInd) = [];
            darkFrame = squeeze(mean(reshape(raw(:,:,:,runInfo.darkFramesInd), ...
                size(raw,1),size(raw,2),[]),3,'omitnan'));

            %Eliminate dark frames from
            if contains(runInfo.system,'EastOIS2')
                raw(:,:,sessionInfo.cam2Chan,:) = double(raw(:,:,sessionInfo.cam2Chan, ...
                    :)) - double(repmat(darkFrame,1,1,numel(sessionInfo.cam2Chan), ...
                    size(raw(:,:,sessionInfo.cam2Chan,:),4)));
                raw(:,:,sessionInfo.cam1Chan,:) = double(raw(:,:,sessionInfo.cam1Chan, ...
                    :)) - double(repmat(darkFrame,1,1,numel(sessionInfo.cam1Chan), ...
                    size(raw(:,:,sessionInfo.cam1Chan,:),4)));
            else
        ...
        if ~isempty(runInfo.fluorChInd)
            ...
            %which channel is being used a reference
        ...
        datahb = procOIS(raw(:,:,runInfo.hbChInd,:),op.dpf,E,isbrain);
        ...
        if ~isempty(runInfo.fluorChInd)
            ...
            datafluor = procFluor(squeeze(raw(:,:,runInfo.fluorChInd,:)),mean( ...
                raw(:,:,runInfo.fluorChInd,:),4,'omitnan'));
            ...
            if  strcmp('EastOIS1_GCaMP',runInfo.system) %gcamp correction
                datafluorCorr = correctHb_differentBeta(datafluor,datahb, ...
                    [E_in(1) E_out(1)],[E_in(2) E_out(2)],dpIn,dpOut,1,1);
            ...
            save(runInfo.saveFluorFile,'xform_datafluor','xform_datafluorCorr', ...
                'op_in', 'E_in', 'op_out', 'E_out','runInfo','-v7.3')
        ...
        %FAD
        if ~isempty(runInfo.FADChInd)
            [op_inFAD, E_inFAD] = getHbOpticalProperties({runInfo.lightSourceFiles ...
                {runInfo.FADChInd}});
            [op_outFAD, E_outFAD] = getHbOpticalProperties(runInfo.FADFiles);
            dpIn = op_inFAD.dpf/2;
            dpOut = op_outFAD.dpf/2;

            dataFAD = procFluor(squeeze(raw(:,:,runInfo.FADChInd,:)),mean(raw ...
                (:,:,runInfo.FADChInd,:),4,'omitnan'));
            dataFAD = smoothImage(dataFAD,runInfo.gbox,runInfo.gsigma);
            xform_dataFAD = affineTransform(dataFAD,I);
            clear raw
            %FAD-Correction
            dataFADCorr = correctHb_differentBeta(dataFAD,datahb,[E_inFAD(1) ...
                E_outFAD(1)],[E_inFAD(2) E_outFAD(2)],dpIn,dpOut,1,1);
            dataFADCorr = smoothImage(dataFADCorr,runInfo.gbox,runInfo.gsigma);
            xform_dataFADCorr=affineTransform(dataFADCorr,I);


            save(runInfo.saveFADFile,'xform_dataFAD','xform_dataFADCorr', ...
                'op_inFAD', 'E_inFAD', 'op_outFAD', 'E_outFAD','runInfo','-v7.3')
        ...
        if ~isempty(sessionInfo.chLaser)
            ...
            save(strcat(runInfo.saveFilePrefix,'-dataLaser.mat'),'xform_datalaser', ...
                'runInfo','-v7.3')
        end
    end
...
%Power Break
            ...
            for contrast=1:numel(runInfo.Contrasts)
                  [whole_spectra_map(:,:,:,contrast),avg_cort_spec(:,contrast ...
                      ),powerMap(:,:,:,contrast),hz, global_sig_for(:,contrast ...
                      ),glob_sig_power(:,contrast)]= PowerAnalysis(squeeze( ...
                      data_full(:,:,:,contrast)),runInfo.samplingRate,xform_isbrain,fft_bool); 
            end
            save(strcat(runInfo.saveFilePrefix,'-Power.mat'),'hz','whole_spectra_map', ...
                'powerMap','global_sig_for','glob_sig_power','avg_cort_spec','-v7.3')
            ...
            data_isa=   nan(size(data_full,1),size(data_full,2),size(data_full,3) ...
                *((2*fRange_ISA(2)*1.25)/runInfo.samplingRate) ,size(data_full,4));
            data_delta= nan(size(data_full,1),size(data_full,2),size(data_full,3) ...
                *((2*fRange_delta(2)*1.25)/runInfo.samplingRate) ,size(data_full,4));
            for ii=1:size(data_full,4)
                data_isa_tmp(:,:,:,ii)=filterData(data_full(:,:,:,ii),fRange_ISA(1 ...
                    ),fRange_ISA(2),runInfo.samplingRate,xform_isbrain);
                data_isa(:,:,:,ii)=resample(squeeze(data_isa_tmp(:,:,:,ii)), ( ...
                    2*fRange_ISA(2)*1.25) ,runInfo.samplingRate,'Dimension',3);

                data_delta_tmp(:,:,:,ii)=filterData(data_full(:,:,:,ii),fRange_delta(1 ...
                    ),fRange_delta(2),runInfo.samplingRate,xform_isbrain);
                data_delta(:,:,:,ii)=resample(squeeze(data_delta_tmp(:,:,:,ii)), ( ...
                    2*fRange_delta(2)*1.25) ,runInfo.samplingRate,'Dimension',3);
            end
            ...
            save(saveSeedFCName,'contrastName','seedCenter','seedFC','seedFCMap', ...
                'seedRadius','-v7.3');
            ...
            if ~isempty(runInfo.fluorChInd)
                ...
                clear fh
                [fh, seedFC, seedFCMap,seedCenter,seedRadius,bilatFCMap]...
                    = qcFC(data_delta,xform_isbrain,runInfo.Contrasts,runInfo, ...
                    fRange_delta,ContFig2Save);
                for contrastInd = 1:numel(ContFig2Save)
                    saveFCQCFigName = [runInfo.saveFCQCFig '-' ...
                        ContFig2Save{contrastInd} '-' fStr ];
                    saveas(fh(contrastInd),[saveFCQCFigName '.png']);
                    close(fh(contrastInd));
                end
                ...
                %Saving
                saveSeedFCName = [runInfo.saveFilePrefix '-seedFC-' fStr];
                save(saveSeedFCName,'contrastName','seedCenter','seedFC', ...
                    'seedFCMap','seedRadius','-v7.3');
                saveBilateralFCName = [runInfo.saveFilePrefix '-bilateralFC-' fStr];
                save(saveBilateralFCName,'contrastName','bilatFCMap','xform_isbrain','-v7.3');
            end
    ...
    elseif runInfo.session =="stim"
            ...
            data_full(:,:,:,3)=squeeze(xform_datahb(:,:,1,:))*10^6+squeeze( ...
                xform_datahb(:,:,2,:))*10^6;
            if ~isempty(runInfo.fluorChInd)
            ...
            save(strcat(runInfo.saveFilePrefix,'-Power.mat'),'hz','whole_spectra_map', ...
                'powerMap','global_sig_for','glob_sig_power','-v7.3')
            ...
            R=mod(size(data_full,3),stimBlockSize); % Check for dropped frames
            %if there is dropped frames
            if R~=0
                pad=stimBlockSize-R;
                disp(['** Non integer number of blocks presented. Padded with ' , ...
                    num2str(pad), ' zeros **'])
                data_full(:,:,end:end+pad,:)=0;
                runInfo.appendedZeros=pad;
            end
            numBlocks = round(size(data_full,3)/(runInfo.blockLen*runInfo.samplingRate));

            % GSR, Filter, Downsample
            data_full = reshape(data_full,size(data_full,1),size(data_full,2),[], ...
                length(runInfo.Contrasts));
            for ii = 1:length(runInfo.Contrasts)
                data_full(:,:,:,ii) =  filterData(squeeze(squeeze(data_full(:,:,:,ii))), ...
                    0.01,5,runInfo.samplingRate);                   
            end
            data_full_BaselineShift= resample(data_full,(2*fRange_delta(2)*1.25), ...
                runInfo.samplingRate,'Dimension',3 ); %resample to 10 Hz
            data_full_gsr_BaselineShift=nan(size(data_full_BaselineShift));
            for ii = 1:length(runInfo.Contrasts)
                data_full_gsr_BaselineShift(:,:,:,ii) = gsr(squeeze(data_full_BaselineShift ...
                    (:,:,:,ii)),xform_isbrain);
            end
            
            %change the sampling rate to fit the resample frequency
            runInfo.samplingRate=(2*fRange_delta(2)*1.25); 
            clear data_full_gsr data_full

            %Reshape into blocks
            %reshape to pixel-pixel-blockSize-numblock-species
            data_full_gsr_BaselineShift = reshape(data_full_gsr_BaselineShift, ...
                size(data_full_gsr_BaselineShift,1),size(data_full_gsr_BaselineShift,2), ...
                [],numBlocks,size(data_full_gsr_BaselineShift,4));
            %reshape to pixel-pixel-blockSize-numblock-species
            data_full_BaselineShift = reshape(data_full_BaselineShift, ...
                size(data_full_BaselineShift,1),size(data_full_BaselineShift,2), ...
                [],numBlocks,size(data_full_BaselineShift,4));

            %Mean subtraction
            meanFrame = squeeze(mean(data_full_BaselineShift(:,:,1:runInfo.stimStartTime* ...
                runInfo.samplingRate,:,:),3));
            data_full_BaselineShift = xform_isbrain.*(data_full_BaselineShift - ...
                permute(repmat(meanFrame,1,1,1,1, size(data_full_BaselineShift,3)), ...
                [1,2,5,3,4]  ));

            meanFrame = squeeze(mean(data_full_gsr_BaselineShift(:,:, ...
                1:runInfo.stimStartTime*runInfo.samplingRate,:,:),3));
            data_full_gsr_BaselineShift = xform_isbrain.*(data_full_gsr_BaselineShift ...
                - permute(repmat(meanFrame,1,1,1,1, size(data_full_gsr_BaselineShift,3)), ...
                [1,2,5,3,4]  ));
            clear  meanFrame
            ...
            [fh,peakMaps]= generateBlockMap_Dynamics(data_full_BaselineShift, ...
                data_full_gsr_BaselineShift,runInfo,numBlocks,peakTimeRange, ...
                xform_isbrain,xform_StimROIMask);
            %save
            ...
            close all
            save([runInfo.saveFilePrefix '-StimResults'],'data_full_gsr_BaselineShift', ...
                'data_full_BaselineShift','-v7.3');
